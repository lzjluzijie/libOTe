...
namespace osuCrypto
{
    ...
    // Compute e = G * e.
    template<typename T>
    void ExConvCode::dualEncode(span<T> e)
    {
        if (e.size() != mCodeSize)
            throw RTE_LOC;
    ...
    void ExConvCode::dualEncode2(span<T0> e0, span<T1> e1)
    {
    ...
    // Compute w = G * e.
    template<typename T>
    void ExConvCode::dualEncode(span<T> e, span<T> w)
    {
        if (e.size() != mCodeSize)
            throw RTE_LOC;
...
//            auto xj1 = xx1[j] ^ xx1[i];
    ...
    template<typename T>
    void ExConvCode::accumulate(span<T> x)
    {
...
#undef CASE
    ...
    template<typename T0, typename T1>
    void ExConvCode::accumulate(span<T0> x0, span<T1> x1)
    {
        PRNG prng(mSeed ^ OneBlock);
